...
***                                                                                                    ***
***   1、核心设备使用ssh，核心设备有华为和华三                                                         ***
***         命令文本：h3c_cmd.txt/hw_cmd.txt                                                           ***
***         地址文本：h3c_list.txt/hw_core_list.txt                                                    ***
***   2、接入设备使用telnet                                                                            ***
***         命令文本：hw_cmd.txt                                                                       ***
***         地址文本：hw_access_list.txt                                                               ***
***   3、如果收集的信息较多，需要在命令文本中增加不分屏显示命令。                                      ***
**********************************************************************************************************''')
Device = input('''
...
if Device == '1':
        ...
        for line in open("h3c_list.txt"):
                ...
                try:
                        ...
                        s1.write(ip + '\n')                             # 把host对应的ip地址记录到tel文本中
                        count_True += 1
                except paramiko.ssh_exception.AuthenticationException:  # 认证失败
                        print(ip, 'Authentication Failed')
                        f1.write(ip + '\n')
                        count_False += 1
                except socket.error:                                    # 网络不可达
                        print(ip, "is unreachable")
                        f1.write(ip + '\n')
                        count_False += 1
if Device == '2':
