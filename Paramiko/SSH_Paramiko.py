...
t1 = open('tel1.txt', 'w')                  # 创建文本文件，w 表示只写,原有文件会被删除覆盖，记录ssh成功的IP
f1 = open('tel2.txt', 'w')
...
for line in open("core_list.txt"):          # 执行一个for循环，定义line 变量，打开core_list.txt文本，默认是r模式
        ...
        try:
                client = paramiko.SSHClient()       # 调用paramike的SSHClient()方法，赋值给变量client
                ...
                client.connect(host, port=ssh_port, username=username, password=password, timeout=10, look_for_keys=False)   # 使用paramiko.SSHClient()的connect()函数进行登陆，地址，端口，用户名和密码，超时时间，look_for_keys=False表示禁用在~/.ssh中搜索私钥文件。
                ...
                time.sleep(2)                       # 等待2s
                for command in open("commands.txt"):      # 循环执行命令，
                        cmd = command.replace('\n', '')
